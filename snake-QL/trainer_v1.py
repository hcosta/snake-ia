...
from snake_v8 import SnakeGame, SCREEN_WIDTH, SCREEN_HEIGHT, STEP_SIZE, REWARD_STUCK_LOOP, POINTS_PER_FOOD
from agent_v1 import Agent

# --- Constantes de Entrenamiento ---
POSSIBLE_ACTIONS = [0, 1, 2, 3]

# Hiperparámetros del Agente (Estrategia 0), estancado entre 250 y 300 puntos
# En este escenario lo mejor hacer un reward shaping y cambiar los incentivos
LEARNING_RATE = 0.1
DISCOUNT_FACTOR = 0.9
EPSILON_START = 1.0
...
def play_with_trained_agent(q_table_path=Q_TABLE_FILENAME, num_episodes=10, visualization_speed=0.001):
    ...
    try:
        ...
        for episode in range(1, num_episodes + 1):
            ...
            while not done:
                ...
                if done:
                    print(
                        f"Episodio {episode} finalizado. Puntuación: {episode_score//10}")
                    env_play.clear()
                    env_play.on_draw()
                    arcade.finish_render()
                    time.sleep(2)
